              ...
              data.map((item,index)=>{
                return(
                  <div key={item.userId+index}>
                     ...
                     <p className='font-medium text-lg'>{(item?.userId?.name)}</p> 
                     <div className='border rounded p-2'>
                                  ...
                                  item?.productDetails.map((product,index)=>{
                                    return(
                                      <div key={product.productId+index} className='flex  gap-3 bg-slate-100'>
                                          <img 
                                            src={product.image[0]}
                                            className='w-28 h-28 bg-slate-200 object-scale-down p-2' alt=''
                                          />
                                          <div>
                                            <div className='font-medium text-lg text-ellipsis line-clamp-1'>{product.name}</div>
                                            <div className='flex items-center gap-5 mt-1'>
                                              <div className='text-lg text-red-500'>{displayVNCurrency(product.price)}</div>
                        ...
                        <div className='font-semibold ml-auto w-fit lg:text-lg'>
                          Total Amount : {displayVNCurrency(item.totalAmount)}
                        </div>
                     </div>
                  </div>
                )
